...
class NPC(Entity):
    ...
    def prep_state_label(self):
        self.state_surface = self.world.game_instance.settings.font.render(f"{self.brain.active_state}", True, (255, 255, 255))
        ...
        if distance > 5:
            ...
            if v_length != 0:
                ...
                if self.movement_direction[0] or self.movement_direction[1]:
                    ...
                    self.rect.topleft = (self.x - self.world.camera.x, self.y - self.world.camera.y)
...
class RangerNPC(NPC):
    ...
    def __init__(self, world, spawn_location):
        super().__init__(world, "ranger_npc")
